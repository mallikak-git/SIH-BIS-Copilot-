...
// Load BIS CSV data
const { loadProducts } = require("./bisData");
...
app.get("/api/evidence/:product", (req, res) => {
  try {
    ...
    console.log("Evidence request received for:", product);

    // Create evidence list if product does not exist yet
...
loadProducts()
  .then((products) => {
    ...
    console.log(`BIS CSV products loaded: ${products.length}`);
    ...
    server.on("error", (error) => {
      ...
      if (error.code === "EADDRINUSE") {
        ...
        console.error(
          "Close the old Node server and start again."
        );
      } else {
        console.error(error);
      }
  ...
  .catch((error) => {
    console.error("");
    console.error("====================================");
    console.error("       BIS CSV LOAD FAILED");
    console.error("====================================");
